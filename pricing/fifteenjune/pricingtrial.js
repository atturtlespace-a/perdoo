...
const panels = {
  ...
  toggleCta:{
    ...
    viewSupreme:document.querySelectorAll('[data-pr="supremePriceViewButton"]') ,
  },
  boxes:{
...
let druser ,dvuser,premiumRegularPrice,supremeRegularPrice,supremeViewOnlyPrice, perEmpPmPre, perEmpPmSup, totalInvPre,totalInvSup, roiPeryearPre,
roiPeryearSup,roiMultiplyValuePre,roiMultiplyValueSup;
...
function totalMonthlyDisplayTextcontent(selector,value,roundTwoDecimalorZeroDecimal){
...
function calculations(preUser, supUser, supView, rC) {
  ...
  totalmonthlysupreme = numberofusers * supremeRegularPrice + numberofviewonly * supremeViewOnlyPrice;
  totalMonthlyDisplayTextcontent(panels.totalMonthly.premium, totalmonthlypremium,true);// Total Per Month Premium
  totalMonthlyDisplayTextcontent(panels.totalMonthly.supreme, totalmonthlysupreme,true);// Total Per Month Supreme (Both)
  perEmpPmPre = totalmonthlypremium / numberofusers;
  perEmpPmSup = totalmonthlysupreme / (numberofusers + numberofviewonly);
  totalMonthlyDisplayTextcontent(panels.perEmployee.premium, perEmpPmPre,false); // Per employee premium
  totalMonthlyDisplayTextcontent(panels.perEmployee.supreme, perEmpPmSup,false);// Per employee supreme
  totalInvPre = totalmonthlypremium * 12;
  totalInvSup = totalmonthlysupreme * 12;
  totalMonthlyDisplayTextcontent(panels.totalInvestment.premium ,totalInvPre, true);
  totalMonthlyDisplayTextcontent(panels.totalInvestment.supreme ,totalInvSup, true);
  roiPeryearPre = rC * numberofusers // ROI Premium
  roiPeryearSup = rC * (numberofusers + numberofviewonly) // ROI Supreme
  totalMonthlyDisplayTextcontent(panels.roiPerYear.premium,roiPeryearPre,true);
  totalMonthlyDisplayTextcontent(panels.roiPerYear.supreme,roiPeryearSup,true);
  roiMultiplyValuePre = roiPeryearPre/totalInvPre;
  roiMultiplyValueSup = roiPeryearSup/totalInvSup;
  panels.roiMultiple.premium.textContent = `${Math.round(roiMultiplyValuePre)}`;
  panels.roiMultiple.supreme.textContent = `${Math.round(roiMultiplyValueSup)}`;
...
// Set the display properties
  ...
  if (numberofviewonly < 1000) {    
    ...
    displayOnlyContent(panels.displayPricing.supremeView, internationalUpToTwoDecimals(supremeViewOnlyPrice));
  } else {
